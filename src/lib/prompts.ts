...
- If web/image search finds existing images, use normal Markdown image URLs, source links, or Grok searched-image render markers such as \`[[render_searched_image with image_id is img-1 size is "LARGE"]]\` or \`<grok:render type="render_searched_image"><argument name="image_id">img-1</argument></grok:render>\`. Prefer the exact image ids supplied by the search/image result (for example \`img-1\`, \`img-2\`, or provider-specific ids). Do not invent image ids.
