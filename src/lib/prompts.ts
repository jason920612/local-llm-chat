...
function executionDirective(): string {
  ...
  const exec = vm
    ? `- "run_code" runs in an ISOLATED per-conversation microVM where you are ROOT — full Linux, internet, apt/pip all available. Use it FREELY for finite commands: compute, install packages, test code, process data, build deliverables. To actually DELIVER a file you produced (report.pdf, data.xlsx, …) you MUST write the marker [[file:EXACT_NAME]] on its own line in your reply — that is what attaches and shows the file. Saying "done" or just naming the file does NOT deliver it; only the marker does (this is separate from send_screenshot and generated images).
- Slow finite tasks are handled FOR you: if a run_code call runs longer than ~${fgSec}s it is AUTOMATICALLY moved to the background and keeps running up to the VM run limit; you are then notified with its full output when it finishes. So NEVER avoid or fake a task because it "takes time" — just run it for real. When that completion notice lists files, you STILL must emit their [[file:NAME]] markers to deliver them. While a run is in the background for this conversation, don't start another run_code until you get the completion notice.
- For persistent SERVICES (servers, bots, watchers, daemons, or any command the user expects to keep running), do NOT use run_code as the long-running process and do NOT wrap commands in tmux/nohup/& yourself. You decide from the user's intent whether a command is a persistent service. Use start_background with kind="service" and pass the normal FOREGROUND command (for example: cd /workspace/app && npm start). The background manager daemonizes it, stores pid/log, keeps it running until kill_background, and will not wake you just because startup succeeded. After starting a service, verify once with read_background_log/list_background or a port check, then answer the user.`
...
- For long-running finite work use start_background kind="task" (auto-woken on completion). For persistent services/servers/watchers use start_background kind="service" with the foreground command; do not wrap it in tmux/nohup/& yourself. Use run_code for quick commands.`;
  return `
...
- Do not avoid mouse exploration. Observations return the current cursor position, screenshots show a red cursor marker when it is visible, and elements/marks include distanceFromCursor {dx,dy,px}. Use action "move" to hover over icons, menus, canvas/image controls, toolbars, cards, or ambiguous elements, then observe/include_screenshot again to read hover text or changed UI before deciding whether to click.
