...
const ACTION_PROGRAM_DESC =
  ...
  "Clicks move a REAL cursor smoothly to the target (human-like, not a synthetic/JS event) on both desktop and browser; use action `move` freely to hover/explore menus, tooltips, cursor position, and image/canvas UI before clicking; set a step's `fast:true` to skip the travel only when you explicitly want instant pointer movement. " +
...
const BROWSER_OBSERVE_TOOL = {
  ...
  description:
    "Observe the isolated VM browser using DOM-derived elements. Returns current URL/title, current cursor position in viewport coordinates, and visible elements with element IDs, text, roles, bounding boxes, and center coordinates. Elements/marks include distanceFromCursor {dx,dy,px} so you know which object is nearest to the pointer. Screenshots include a visible red cursor marker when the cursor is inside the viewport. Set mark=true for Set-of-Mark grounding on the browser viewport: detector + DOM + OCR boxes are overlaid with numbered marks, useful for text-less icons/canvas/image controls. Prefer this over raw coordinate computer_observe for websites. NOTE: for ordinary clickable links/buttons, the DOM-sourced marks (and dom_* element ids) are the most reliable target even when the visible glyph is tiny (e.g. an upvote ▲) — prefer them over detector marks for clicking.",
  parameters: {
    type: "object",
    properties: {
      include_screenshot: {
